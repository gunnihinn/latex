...
= \frac{\Vol(S(V))}{\binom{n+d-1}{d}} \id.
$$
\end{prop}

\begin{proof}
The integral is well-defined as the integrand is invariant under $S_d$.
Let's denote the map defined by the integral by $L$.
If $g \in \End V$ is unitary with respect to $h$ we have
$h(g x, \ov v) = h(x, \ov{g^{-1} v})$
and $|\!\det g| = 1$
so the change of basis formula implies that $L g^d = g^d L$.
As $\Sym^d V$ is an irreducible representation of the unitary group, this
implies that $L = \lambda \id$ for some scalar $\lambda$.
...
\begin{coro}
\begin{align*}
\int_{S(V)} H(v) d\sigma(v)
&= \frac{\Vol(S(V))}{\binom{n+1}{2}} s.
\\
\int_{S(V)} H(v)^2 d\sigma(v)
&= \frac{2 \Vol(S(V))}{\binom{n+1}{2}}(|R|^2 + s^2).
\end{align*}
\end{coro}


\begin{proof}
If $f \in \End \Sym^2 V$ then Proposition~\ref{prop:int-id} implies that
$$
\int_{S(V)} h^{\odot 2}(f(v \odot v), \ov{v \odot v}) d\sigma(v)
...
The Frobenius norm of $f$ is $|R|^2 = |f|^2 = \tr(f^\dagger f)$, where
$f^\dagger$ is the adjoint of $f$ and
\end{proof}



\begin{prop}
If $R$ is an algebraic K\"ahler curvature tensor with $H = 0$ then $R = 0$.
\end{prop}

\begin{proof}
If $f$ is an endomorphism of $\Sym^2 V$, then $|f|^2 = \tr f^\dagger f$.
Then
\begin{align*}
\frac{\Vol(S(V))}{\binom{n+1}{2}} |f|^2
&= \int_{S(V)} h(f^\dagger f(x \odot x), \ov{x \odot x}) d\sigma
\\
&= \int_{S(V)} h(f(x \odot x), \ov{f(x \odot x)}) d\sigma
= \int_{S(V)} |f(x \odot x)|^2 d\sigma.
\end{align*}
If $q$ is the Hermitian form that $R$ defines on $\Sym^2 V$ we set $f =
h^{-1}q$. Then $|f|^2 = |q|^2$ and
\[
\frac{\Vol(S(V))}{\binom{n+1}{2}} |q|^2
= \int_{S(V)} |q(x \odot x, x \odot x)|^2 d\sigma
= \int_{S(V)} |H(x)|^2 d\sigma.
\qedhere
\]
\end{proof}
...
$$
in an orthonormal basis $(e_j)$, so it is positive-definite if $R$ is Griffiths
positive.
...
$$
in an orthonormal basis, so it is positive if $r$ is positive.

Suppose finally that $H > 0$.
If $q$ is the Hermitian form that $R$ defines on $\Sym^2 V$, then
\[
\frac{\Vol(S(V))}{\binom{n+1}{2}} s
= \int_{S(V)} q(x \odot x, \ov{x \odot x}) d\sigma
= \int_{S(V)} H(x) d\sigma.
\qedhere
\]
\end{proof}
