...
$$
\displaylines{
H(e_j + e_k) + H(e_j - e_k) + H(e_j + i e_k) + H(e_j - i e_k)
= H(e_j) + H(e_k) + 2 B(e_j, e_k)
}
$$
for $j \not= k$.
If $H \geq 0$ we get
\begin{align*}
0
&\leq \sum_{j\not=k} H(e_j) + H(e_k) + 2 B(e_j, e_k)
\\
&= \sum_{j,k} H(e_j) + H(e_k) + 2 B(e_j, e_k)
- 4 \sum_{j} H(e_j)
\\
&= 2n \sum_j H(e_j) + 2 s - 4 \sum_j H(e_j)
= 2s + 2(n-2) \sum H(e_j).
\end{align*}
So for a surface we get $2s \geq 0$.
...
+ |t|^2 (R(u, \bar u, v, \bar v) + R(v, \bar v, u, \bar u))
