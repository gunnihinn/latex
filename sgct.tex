...
The first Bianchi identity can only be stated for metrics on the tangent bundle, as it involves the torsion. The second Bianchi identity is true for the curvature form $\Theta$ of an arbitrary connection $D$ on a vector bundle $E$, where it says that $D_{\End E} \Theta = 0$.
...
We often view a curvature form as a Hermitian form on $E \otimes T_X$. This corresponds here to the space of curvature tensors of Hermitian metrics. This space then has dimension $(n^2)^2 = n^4$. We do have that $\binom{n+1}{2}^2 \leq n^4$ with equality if and only if $n = 1$, which is a nice sanity check. The ratio of the dimensions tends to $1/4$ as $n \to \infty$; in one sense we can expect slightly more than one tensor out of four to be a K\"ahler curvature tensor, in another almost no tensors are the ones of a K\"ahler metric.



TODO: Holomorphic sectional curvature determines curvature tensor. Try to use polarization to show that $H$ determines bisectional curvature, which is the restriction of $R$ to a diagonal, thus it determines the whole thing.
...
An easy way to construct elements of $\cc R$ is to pick a basis for $V$ and just write down some tensors. This is in some way unsatisfactory because we'd like to see curvature tensors that arise in the wild. To do so we'd like to have coordinate-invariant ways of constructing elements of $\cc R$. We point out two of those here.

Suppose we have a Hermitian form $h$ on $V$. It induces a Hermitian form on $S^2 V$, defined by
$$
(h \odot h)(x \odot y, z \odot w)
= h(x,z)h(y,w) + h(z,y)h(x,w).
$$
This is the complex-geometric version of the Kulkarni--Nomizu product. If $g$ and $h$ are Hermitian forms on $V$ we can polarize this identity and get a Hermitian form on $S^2V$ by setting
$$
\displaylines{
(g \odot h)(x \odot y, z \odot w)
  = g(x,y) h(z,w) + g(x,w) h(z,y)
  \hfill\cr\hfill{}
  + g(z,y) h(x,w) + g(z,w) h(x,y).
}
$$
...
\begin{prop}
  ...
  Suppose $h$ is a Hermitian inner product on $V$. Then $h \odot h$ has constant holomorphic scalar curvature equal to $2$, and is Griffiths-positive.
\end{prop}
