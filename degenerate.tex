...
If this operation should be called anything, it is clearly a \emph{reformation} of the original space, as it removes all the degeneracy from the Hermitian form.
...
It's possible to define this form by passing to quotients as well, but the construction is a little less clear. We consider the commutative diagram
\[
\begin{tikzcd}
0 \ar[r] &
S \ar[r,"j"] \ar[d,"\pi_S"] &
...
(V / \Ker b_V) / (S / \Ker b_S) \ar[r] &
0
\end{tikzcd}
\]
where the horizontal sequences are exact,
from which we arrow chase a surjective morphism $\pi_Q : Q \to (V / \Ker b_V) / (S / \Ker b_S)$ that makes the relevant squares commute. The space $V / \Ker b_V$ has a non-degenerate Hermitian form, so we get a non-degenerate Hermitian form on its quotient in the usual way. The form on $Q$ is then defined by pullback by $\pi_Q$.
...
TODO: Prove something. Possibly that $\Hom(V,W) \cong V^* \otimes W$ behaves well with respect to this. Talk about what the form on $V^*$ is.
